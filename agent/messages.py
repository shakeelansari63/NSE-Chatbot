...
def get_execution_system_message():
    return "\n".join(
        [
            ...
            "Finally, when responding to user, respond in rich text markdown with with tabular data if possible.",
            f"NOTE: **Current Date Time is: {datetime.now().strftime('%A, %d %B %Y')}**",
        ]
    )
...
def get_shortlist_message(tools: list[str], user_conversation: list[BaseMessage]):
    return "\n".join(
        [
            ...
            "AND Always Select the Tools which helps to search Companies in NSE Database as many tools need stock symbol but user may give only stock name. ",
            ...
            '\tAnswer: ["search_stock", "get_stock_price"]',
            "Example 3: ",
            "\tQuestion: Hello !!",
            "\tAnswer: []",
